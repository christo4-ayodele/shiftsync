...
import { MAX_PENDING_SWAP_REQUESTS, DROP_REQUEST_EXPIRE_HOURS_BEFORE_SHIFT } from '@/lib/utils/constants'
  ...
  let query = supabase
    ...
    .select(`
      *,
      ...
      target_staff:profiles!target_staff_id(*),
      target_assignment:shift_assignments!target_assignment_id(
        *,
...
export async function createSwapRequest(data: {
  ...
  if (data.type === 'drop' && assignment?.shift) {
    ...
    expiresAt = subHours(shiftStart, DROP_REQUEST_EXPIRE_HOURS_BEFORE_SHIFT).toISOString()
  }
...
export async function acceptSwapRequest(swapRequestId: string) {
  ...
  const { data: request } = await supabase
    .from('swap_requests')
    .select('*, requesting_assignment:shift_assignments!requesting_assignment_id(staff_id)')
    .eq('id', swapRequestId)
    .single()

  if (!request) throw new Error('Swap request not found')
  if (request.status !== 'pending_peer') {
    return { success: false, message: 'This swap request is no longer pending peer acceptance.' }
  }
  ...
  if (requesterId) {
    await supabase.from('notifications').insert({
      ...
      message: 'Your swap request has been accepted and is now awaiting manager approval.',
      link: '/dashboard/swap-requests',
      is_read: false,
      delivery_method: 'in_app',
    })
...
export async function approveSwapRequest(swapRequestId: string) {
  ...
  const { data: request } = await supabase
    ...
    .select(`
      *,
      requesting_assignment:shift_assignments!requesting_assignment_id(*, shift:shifts(*), profile:profiles(*)),
      target_staff:profiles!target_staff_id(*)
    `)
    .eq('id', swapRequestId)
    .single()
  ...
  if (request.status !== 'pending_manager') {
    return { success: false, message: 'This request is not pending manager approval.' }
  }
...
export async function rejectSwapRequest(swapRequestId: string, reason?: string) {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) throw new Error('Not authenticated')
  ...
  if (requesterId) {
    await supabase.from('notifications').insert({
      ...
      message: reason ? `Your request was rejected: ${reason}` : 'Your swap/drop request was rejected.',
      link: '/dashboard/swap-requests',
      is_read: false,
      delivery_method: 'in_app',
    })
...
export async function claimDroppedShift(swapRequestId: string) {
  ...
  if (request.type !== 'drop' || request.status !== 'pending_manager') {
    return { success: false, message: 'This shift is not available for pickup.' }
  }
  ...
  if (shiftData?.shift?.location_id) {
    ...
    if (managers) {
      const notifications = managers.map(m => ({
        ...
        message: 'A staff member has volunteered to pick up a dropped shift. Approval needed.',
        link: '/dashboard/swap-requests',
        is_read: false,
