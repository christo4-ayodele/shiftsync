...
import { format, startOfWeek, endOfWeek, subWeeks, parseISO, differenceInHours } from 'date-fns'
import { ChevronLeft, ChevronRight, BarChart3, PieChart as PieIcon, TrendingUp } from 'lucide-react'
...
export default function AnalyticsPage() {
  ...
  const fetchAnalytics = useCallback(async () => {
    ...
    const filtered = (assignments || []).filter(a => {
      ...
      if (selectedLocation !== 'all' && a.shift.location_id !== selectedLocation) return false
      return true
    })
    ...
    for (const a of filtered) {
      ...
      const hours = differenceInHours(parseISO(a.shift.end_time), parseISO(a.shift.start_time))
      ...
      const week = format(startOfWeek(parseISO(a.shift.start_time), { weekStartsOn: 1 }), 'MMM d')
  ...
  if (user?.role === 'staff') {
    return <div className="text-center py-12 text-muted-foreground">Access restricted to managers and admins.</div>
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Fairness metrics and scheduling insights</p>
        </div>
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-lg">Hours Distribution by Staff</CardTitle>
              </div>
              <CardDescription>Total hours worked per staff member — check for equitable scheduling</CardDescription>
            </CardHeader>
            <CardContent>
              {hoursData.length === 0 ? (
              ...
              {hoursData.length > 1 && (
                <div className="mt-4 flex gap-4 text-sm text-muted-foreground">
                  <span>Min: {Math.min(...hoursData.map(d => d.hours))}h</span>
                  <span>Max: {Math.max(...hoursData.map(d => d.hours))}h</span>
                  <span>Avg: {(hoursData.reduce((s, d) => s + d.hours, 0) / hoursData.length).toFixed(1)}h</span>
                  <span>
                    Spread: {(Math.max(...hoursData.map(d => d.hours)) - Math.min(...hoursData.map(d => d.hours))).toFixed(1)}h
                  </span>
                </div>
              )}
            </CardContent>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Distribution of hours across skill requirements</CardDescription>
              </CardHeader>
              <CardContent>
                {skillData.length === 0 ? (
                  ...
                  <ResponsiveContainer width="100%" height={250}>
                    <PieChart>
                      <Pie data={skillData} cx="50%" cy="50%" outerRadius={80} dataKey="value" label={({ name, value }) => `${name}: ${value}h`}>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Total scheduled hours per week</CardDescription>
              </CardHeader>
              <CardContent>
