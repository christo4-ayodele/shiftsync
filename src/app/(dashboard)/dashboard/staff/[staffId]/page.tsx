...
import { getStaffMember, updateStaffSkills, updateStaffLocations, getSkills, getLocations } from '@/lib/actions/staff'
...
export default function StaffDetailPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setSelectedSkills(memberData?.staff_skills?.map((ss: any) => ss.skill_id) || [])
      setSelectedLocations(
        memberData?.staff_locations?.filter((sl: any) => !sl.decertified_at).map((sl: any) => sl.location_id) || []
      )
  ...
  if (!member) {
    return <div className="text-center py-12 text-muted-foreground">Staff member not found.</div>
  }
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <Separator />
            <div>
              <p className="text-sm text-muted-foreground mb-1">Max weekly hours</p>
              <p className="font-medium">{member.desired_weekly_hours || 40}h</p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-2">
              {DAYS.map((day, idx) => {
                const dayAvail = availability.filter((a: any) => a.day_of_week === idx)
                return (
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Assign or remove skill certifications</CardDescription>
          </CardHeader>
          <CardContent>
                ...
                <div key={skill.id} className="flex items-center gap-2">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) => {
                      ...
                      else setSelectedSkills(s => s.filter(id => id !== skill.id))
                    }}
                  />
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Which locations this staff member can work at</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
                ...
                <div key={loc.id} className="flex items-center gap-2">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) => {
                      ...
                      else setSelectedLocations(l => l.filter(id => id !== loc.id))
                    }}
                  />
                  <Label htmlFor={`loc-${loc.id}`} className="flex items-center gap-1.5">
