...
import {
  ...
  Send, Edit, Trash2, Users, AlertTriangle, Clock, Check, X
} from 'lucide-react'
import { format, startOfWeek, endOfWeek, addWeeks, subWeeks, addDays, parseISO, isWithinInterval } from 'date-fns'
import { formatInTimezone, formatTimeInTimezone, getShiftDurationHours } from '@/lib/utils/timezone'
...
export default function SchedulePage() {
  ...
  useEffect(() => {
    async function fetchLocations() {
      ...
      if (user.role === 'admin') {
        ...
        setLocations(data || [])
        if (data && data.length > 0 && !selectedLocation) setSelectedLocation(data[0].id)
  ...
  async function handleCreateShift() {
    ...
    setNewShift({ required_skill_id: '', start_time: '09:00', end_time: '17:00', headcount_needed: 1, notes: '' })
    fetchScheduleData()
  }
    ...
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-7 gap-2">
          {weekDays.map((day) => {
            ...
            return (
              <Card key={dayStr} className={`min-h-[160px] ${isToday ? 'ring-2 ring-primary' : ''}`}>
                <CardHeader className="p-2 pb-1">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-semibold">
                      {format(day, 'EEE')}
                    </span>
                    <span className={`text-xs ${isToday ? 'bg-primary text-primary-foreground rounded-full px-1.5 py-0.5' : 'text-muted-foreground'}`}>
                      {format(day, 'd')}
                    </span>
                  </div>
                </CardHeader>
                <CardContent className="p-2 pt-0 space-y-1">
                  ...
                  {schedule && (
                    <button
                      onClick={() => { setSelectedDay(day); setShowCreateShift(true) }}
      ...
      <Dialog open={showCreateShift} onOpenChange={setShowCreateShift}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              Create Shift — {selectedDay ? format(selectedDay, 'EEE, MMM d') : ''}
            </DialogTitle>
            <DialogDescription>
          ...
          <div className="space-y-4">
            <div>
              ...
              <Select value={newShift.required_skill_id} onValueChange={v => setNewShift(s => ({ ...s, required_skill_id: v }))}>
                <SelectTrigger>
            ...
            <div>
              ...
              <Input
                ...
                onChange={e => setNewShift(s => ({ ...s, headcount_needed: parseInt(e.target.value) || 1 }))}
              />
            </div>
            <div>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCreateShift} disabled={!newShift.required_skill_id}>
              Create Shift
            </Button>
          </DialogFooter>
        </DialogContent>
...
function ShiftCard({ shift, timezone, onDelete, isDraft }: {
  shift: any
  timezone: string
  onDelete: () => void
  isDraft: boolean
  ...
  const skillColor = SKILL_COLORS[skillName.toLowerCase()] || 'bg-gray-100 text-gray-800'
  ...
  return (
    <div className="p-1.5 rounded border text-xs space-y-1 bg-card hover:bg-muted/30 transition-colors group">
      <div className="flex items-center justify-between">
        <Badge variant="secondary" className={`text-[10px] px-1 py-0 ${skillColor}`}>
          {skillName}
        </Badge>
        {isDraft && (
      ...
      {shift.shift_assignments?.filter((a: any) => a.status === 'assigned').map((a: any) => (
        <p key={a.id} className="text-[10px] text-muted-foreground truncate pl-1">
          • {a.profile?.full_name}
        </p>
      ))}
    </div>
