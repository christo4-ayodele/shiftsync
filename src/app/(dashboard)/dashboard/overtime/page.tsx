...
import { AlertTriangle, Clock, TrendingUp, Users, ChevronLeft, ChevronRight } from 'lucide-react'
import { format, startOfWeek, endOfWeek, addWeeks, subWeeks, parseISO, differenceInHours } from 'date-fns'
...
export default function OvertimePage() {
  ...
  const fetchHours = useCallback(async () => {
    ...
    for (const a of assignments || []) {
      ...
      if (selectedLocation !== 'all' && a.shift.location_id !== selectedLocation) continue
  ...
  const warningCount = staffHours.filter(s => s.has_warning && !s.has_violation).length
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Monitor weekly hours and prevent overtime violations</p>
        </div>
      ...
      {loading ? (
        ...
        <Card><CardContent className="py-10 text-center text-muted-foreground">No staff scheduled this week.</CardContent></Card>
      ) : (
        <Card>
          <CardContent className="p-0">
            <Table>
              ...
              <TableBody>
                {staffHours.map(s => {
                  const pct = Math.min(100, (s.weekly_hours / OVERTIME_THRESHOLDS.WEEKLY_LIMIT_HOURS) * 100)
                  return (
                    <TableRow key={s.id}>
                      <TableCell className="font-medium">{s.name}</TableCell>
                      <TableCell>{s.shifts_count}</TableCell>
                      <TableCell>
                        <span className={s.has_violation ? 'text-red-600 font-bold' : s.has_warning ? 'text-orange-600 font-semibold' : ''}>
                          {s.weekly_hours.toFixed(1)}h
                        </span>
                        <span className="text-muted-foreground text-xs"> / {OVERTIME_THRESHOLDS.WEEKLY_LIMIT_HOURS}h</span>
                      </TableCell>
                      ...
                      <TableCell>
                        {s.has_violation ? (
                          ...
                          <Badge variant="outline" className="text-[10px] text-orange-600 border-orange-300">Warning</Badge>
                        ) : (
                          <Badge variant="outline" className="text-[10px] text-green-600 border-green-300">OK</Badge>
                        )}
                      </TableCell>
                    </TableRow>
                  )
