...
export default function MyShiftsPage() {
  ...
  const now = new Date()
  const upcoming = shifts.filter(s => isAfter(parseISO(s.shift?.start_time), now))
  const past = shifts.filter(s => isBefore(parseISO(s.shift?.start_time), now))
  ...
  async function handleDrop() {
    ...
    toast.success('Drop request submitted — shift will be available for pickup for 24h')
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-muted-foreground">{upcoming.length} upcoming shifts</p>
      </div>
      ...
      <Tabs defaultValue="upcoming">
        ...
        <TabsContent value="upcoming" className="space-y-2 mt-4">
          {upcoming.length === 0 ? (
            <Card><CardContent className="py-10 text-center text-muted-foreground">No upcoming shifts</CardContent></Card>
          ) : (
                  ...
                  <CardContent className="p-4">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      ...
                      <div className="flex gap-2">
                        <Button size="sm" variant="outline" onClick={() => openSwapDialog(assignment)}>
                          <ArrowRightLeft className="h-3.5 w-3.5 mr-1" /> Swap
                        </Button>
                        <Button size="sm" variant="outline" className="text-destructive" onClick={() => openDropDialog(assignment)}>
        ...
        <TabsContent value="past" className="space-y-2 mt-4">
          {past.length === 0 ? (
            <Card><CardContent className="py-10 text-center text-muted-foreground">No past shifts</CardContent></Card>
          ) : (
                ...
                <Card key={assignment.id} className="opacity-60">
                  <CardContent className="p-3">
                    <div className="flex items-center gap-3 text-sm">
                      ...
                      <Badge variant="outline" className="text-[10px]">{location?.name}</Badge>
                    </div>
                  </CardContent>
                </Card>
              )
      ...
      <Dialog open={showSwapDialog} onOpenChange={setShowSwapDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Request to swap this shift with a coworker</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
      ...
      <Dialog open={showDropDialog} onOpenChange={setShowDropDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>The shift will be posted as open for 24 hours before manager review</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              <Label>Reason (optional)</Label>
              <Textarea value={dropReason} onChange={e => setDropReason(e.target.value)} placeholder="Why do you want to drop this shift?" />
            </div>
          </div>
          <DialogFooter>
