...
export default function OnDutyPage() {
  ...
  const totalOnDuty = onDutyShifts.reduce(
    (sum, s) => sum + (s.shift_assignments?.filter((a: any) => a.status === 'assigned').length || 0), 0
  )
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Real-time view of who&apos;s working right now</p>
        </div>
          ...
          <Card>
            ...
            <CardContent>
              {onDutyShifts.length === 0 ? (
                <p className="text-center text-muted-foreground py-6">No one is currently on duty</p>
              ) : (
                <div className="space-y-3">
                  {onDutyShifts.map(shift => {
                    ...
                    const assigned = shift.shift_assignments?.filter((a: any) => a.status === 'assigned') || []
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs text-muted-foreground flex items-center gap-1">
                              <MapPin className="h-3 w-3" />{shift.location?.name}
                            </span>
                          </div>
                          <Badge variant="secondary">{assigned.length}/{shift.headcount_needed}</Badge>
                        </div>
                        <div className="flex gap-2 flex-wrap">
                          {assigned.map((a: any) => (
                            <div key={a.id} className="flex items-center gap-1.5 bg-muted px-2 py-1 rounded text-sm">
              ...
              <CardContent>
                <div className="space-y-2">
                  {upcomingShifts.map(shift => {
                    ...
                    return (
                      <div key={shift.id} className="flex items-center justify-between border rounded p-2 text-sm">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-muted-foreground">{shift.location?.name}</span>
                        </div>
