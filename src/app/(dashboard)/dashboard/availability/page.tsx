...
import { getAvailability, setRecurringAvailability, addAvailabilityException, deleteAvailabilityException } from '@/lib/actions/availability'
...
export default function AvailabilityPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        if (dayRec) {
          schedule[i] = { start: dayRec.start_time?.slice(0, 5) || '09:00', end: dayRec.end_time?.slice(0, 5) || '17:00', available: true }
        } else {
          schedule[i] = { start: '09:00', end: '17:00', available: false }
        }
      }
  ...
  async function handleSaveWeekly() {
    ...
    await setRecurringAvailability(entries.map(e => ({ ...e, timezone: Intl.DateTimeFormat().resolvedOptions().timeZone })))
    setSaving(false)
  ...
  if (loading) {
    return <div className="space-y-4">{Array.from({ length: 3 }).map((_, i) => <Skeleton key={i} className="h-20" />)}</div>
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Set your recurring weekly availability and any exceptions</p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Toggle each day and set your available hours</CardDescription>
        </CardHeader>
        <CardContent>
                  ...
                  <div className="w-28 flex items-center gap-2">
                    <Switch
                      ...
                      onCheckedChange={(checked) =>
                        setWeeklySchedule(s => ({ ...s, [idx]: { ...s[idx], available: checked } }))
                      }
                    />
                    <span className={`text-sm font-medium ${!slot.available ? 'text-muted-foreground' : ''}`}>
                      {day}
                    </span>
                  </div>
                  {slot.available && (
                    <div className="flex items-center gap-2">
                      <Input
                        ...
                        className="w-32"
                        onChange={e =>
                          setWeeklySchedule(s => ({ ...s, [idx]: { ...s[idx], start: e.target.value } }))
                        }
                      />
                      ...
                      <Input
                        ...
                        className="w-32"
                        onChange={e =>
                          setWeeklySchedule(s => ({ ...s, [idx]: { ...s[idx], end: e.target.value } }))
                        }
                      />
                    </div>
                  )}
      ...
      <Card>
        ...
        <CardContent>
          {exceptions.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-4">No exceptions set</p>
          ) : (
            <div className="space-y-2">
              {exceptions.map(exc => (
                <div key={exc.id} className="flex items-center justify-between border rounded-lg p-3">
                  <div>
                    <div className="flex items-center gap-2">
                      <Calendar className="h-4 w-4" />
                      <span className="font-medium">{format(new Date(exc.specific_date), 'EEE, MMM d, yyyy')}</span>
                    ...
                    {exc.reason && (
                      <p className="text-xs text-muted-foreground mt-1 ml-6 italic">{exc.reason}</p>
                    )}
                  </div>
                  <Button size="icon" variant="ghost" className="text-destructive" onClick={() => handleDeleteException(exc.id)}>
      ...
      <Dialog open={showExceptionDialog} onOpenChange={setShowExceptionDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Override your availability for a specific date</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-2">
              <Switch checked={exceptionAvailable} onCheckedChange={setExceptionAvailable} />
              <Label>{exceptionAvailable ? 'Available (with custom hours)' : 'Unavailable (day off)'}</Label>
            </div>
            {exceptionAvailable && (
              <div className="flex items-center gap-2">
                <div>
            ...
            <div>
              ...
              <Input placeholder="e.g. Doctor appointment" value={exceptionReason} onChange={e => setExceptionReason(e.target.value)} />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowExceptionDialog(false)}>Cancel</Button>
            <Button onClick={handleAddException} disabled={!exceptionDate}>Add Exception</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
